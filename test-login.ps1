...
Write-Host "Testing login endpoint at https://metaverseedu.in/workflow/api/auth/login"
...
$headers = @{
    ...
    "Origin" = "http://example.com"  # Test cross-origin request
}

# Send the request
try {
    $response = Invoke-RestMethod -Uri "https://metaverseedu.in/workflow/api/auth/login" -Method Post -Body $body -Headers $headers -ContentType "application/json" -ErrorAction Stop
